...
def instructor_modules(request, slug):
    ...
    return render(request, 'courses/../templates/instructor_module_list.html', {'course': course, 'modules': modules})
def instructor_components(request, id):
    module = Module.objects.get(id=id)
    components = Component.objects.filter(Module_id = module.id)
    return render(request, 'courses/../templates/instructor_module_detail.html', {'components': components, 'module': module})
